...
|---|---|---|---|---|
| 1.1 | ~~**VeriFactu: RegistroAlta/Anulación XML**~~ **HECHO** (2026-06-14): `services/billing/registro_facturacion.py` genera `RegFactuSistemaFacturacion` (Alta + Anulación) reutilizando el `payload_canonico`/huella de la cadena (consistencia garantizada); valida contra los XSD oficiales AEAT (offline). +8 tests. Falta solo el ENVÍO real (1.3, bloqueado por cert). | HECHO | `services/billing/registro_facturacion.py` | roadmap_impl, depth-audit |
| 1.2 | ~~**VeriFactu: QR en factura**~~ **HECHO** (verificado 2026-06-14): QR real generado con `reportlab.graphics.barcode.qr.QrCodeWidget` en `pdf/_invoice_sections.py:343` (`_verifactu_qr_block`), insertado en el PDF (`pdf/invoices.py:181`), datos `{huella, verify_url}` en `billing/queries.py:150`, y endpoint público `GET /api/v1/verify/{huella}` funcional (`verify.py`). No usa dep `qrcode` ni el archivo `qr_generator.py` que nombraba el roadmap. | HECHO | `services/pdf/_invoice_sections.py`, `routes/verify.py` | roadmap_impl |
...
| 1.4 | ~~**XSD oficiales AEAT**~~ **HECHO** (2026-06-14): 7 XSD oficiales VeriFactu (+xmldsig) en `services/aeat/xsd/` + `scripts/update_aeat_xsd.py` para refrescarlos; `xsd_validation.py` ya validaba si existían. `validate_verifactu_xml` valida contra `SuministroLR.xsd` offline (resolver para xmldsig). | HECHO | `services/aeat/xsd/`, `scripts/update_aeat_xsd.py` | roadmap_impl |
...
| 1.6 | ~~**Auto-update: config publish real** (owner/repo)~~ **HECHO en el JSON** (verificado 2026-06-14): `desktop/package.json:50-54` ya tiene `publish` real (github, owner `Llicklair`, repo `Automatiza-Core`), no placeholder. **Pendiente solo el REBUILD** del instalador para hornear `app-update.yml` con esos valores (el `.exe` actual tiene owner placeholder). Movido a la sección "Auto-update / Release" de `todo.md`. | HECHO (falta rebuild) | `desktop/package.json` | plan-gtm, pilot |
| 1.7 | **System tray invisible en runtime** (código existe, no aparece) | PENDIENTE (bug) | `desktop/tray-manager.js` | auditoria_06-10 |
| 1.8 | ~~**BYOK: paso de API key en onboarding**~~ **HECHO** (2026-06-14): paso `llm_config` añadido al wizard (`step_llm_config`, mig `0059`) — obligatorio para completar; se **auto-marca** desde la readiness real de IA del tenant (`sync_llm_config_step`). La clave se guarda con el mecanismo BYOK ya existente (`tenant_service`/`encrypted_keys`). +4 tests. Pendiente: el componente de UI del wizard (es 2.11). | HECHO (backend) | `services/onboarding/wizard.py`, `routes/onboarding_wizard.py` | plan-gtm |
...
| 2.9 | Firma de documentos de gestoría: **PDF server-side HECHO** (2026-06-11) — `GET /hr/documents/{id}/pdf` (con folio, xhtml2pdf) + descarga en `DocumentCard`; estos bytes alimentan `signing.autofirma.init` (la infra de firma ya es agnóstica al contenido). **Falta**: botón "Firmar" en UI (handshake `afirma://` + polling; requiere AutoFirma instalado para probar e2e). | PARCIAL (PDF hecho) | `services/hr/commands.py`, `routes/hr_documents.py` | scoping docs |
