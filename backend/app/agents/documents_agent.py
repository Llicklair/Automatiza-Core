"""
Agente de Documentos — Autónomo con LangGraph.

El LLM decide qué herramientas usar según la intención del usuario:
  - Clasificar un documento subido → classify_document
  - Buscar documentos existentes → list_tenant_documents / get_document_content
  - Generar embeddings para RAG → generate_embeddings
  - Crear nuevos documentos → create_document
...
async def documents_agent_node(state: AgentState):
    ...
    if "messages" not in state or not state["messages"]:
        sys_msg = SystemMessage(
            content=DOCUMENTS_SYSTEM_PROMPT.format(
    ...
    result_log = StepResult(
        ...
        description="Procesando solicitud de documentos...",
        status="completed",
        action_taken=(
            "Invocando herramientas de documentos"
            if response.tool_calls
            else "Asistencia de documentos completada."
        ),
    )
...
def documents_finalize_node(state: AgentState):
    """Cierra el flujo del agente de documentos."""
    ...
    final_result = StepResult(
        ...
        action_taken=(
            ...
            else "Operación de documentos completada."
        ),
    )
...
workflow.add_node("documents_agent", documents_agent_node)
workflow.add_node("tools", ToolNode(tools))
workflow.add_node("finalize", documents_finalize_node)

workflow.set_entry_point("documents_agent")
workflow.add_conditional_edges("documents_agent", tools_condition)
workflow.add_edge("tools", "documents_agent")
