...
async def banking_agent_node(state: AgentState):
    if "messages" not in state or not state["messages"]:
        sys_msg = make_cached_system_message(BANKING_SYSTEM_PROMPT.format(tenant_id=state.get("tenant_id", "")))
...
workflow.add_conditional_edges("banking_agent", tools_condition, {"tools": "tools", "__end__": "finalize"})
