...
async def run_email_agent(
    ...
    if not is_mock:
            ...
            try:
                if provider == "gmail":
                    ...
                    try:
                        ...
                        return (
                            f"Correo enviado via Gmail{attach_msg}\nAsunto: {subject}\nPara: {to}"
                        )
                    finally:
                        await client.close()
                elif provider == "outlook":
                    ...
                    try:
                        ...
                        return (
                            f"Correo enviado via Outlook{attach_msg}\nAsunto: {subject}\nPara: {to}"
                        )
                    finally:
                        await client.close()
                else:  # imap/smtp
        ...
        tools_list = build_tools_list(check_inbox_real, check_unread_real, send_email_real)
    else:
