                ...
                try:
                    ...
                    for idx, step in enumerate(_cached_data.get("steps", [])):
                        ...
                        if agent not in VALID_DOMAINS:
                            logger.warning("[PLAN] Caché contiene agente inválido '%s', descartando entrada", agent)
                            _cache_valid = False
                            break
...
async def validate_node(state: OrchestratorState) -> OrchestratorState:
    ...
    invalid = [s["id"] for s in plan if s.get("agent") not in VALID_DOMAINS and s.get("agent") != "node_engine"]
    if invalid:
...
async def dispatch_node(state: OrchestratorState) -> OrchestratorState:
    ...
    for idx, step in enumerate(plan):
        ...
        if dep_failed:
            logger.warning("[ORCHESTRATOR] Omitiendo paso '%s' (%s): dependencias fallidas %s", step["id"], step["agent"], dep_failed)
        ...
        for attempt in range(2):  # 1 intento original + 1 retry
            try:
                ...
                result = {
                    ...
                    "agent": agent_name,
                    "success": False,
                    "output": {"action": "timeout", "error": f"El agente '{agent_name}' no respondió en 120 segundos (2 intentos)"},
                ...
                if attempt == 0 and (isinstance(e, (ConnectionError, OSError)) or is_rate_limit):
                    ...
                    logger.warning("[ORCHESTRATOR] Error transitorio en '%s': %s. Reintentando en %ds...", agent_name, type(e).__name__, wait)
    ...
    for idx, subtask, result in gathered:
        ...
        async def _safe_log(_result=result, _subtask=subtask, _agent=agent_name, _action=action_str):
            async with AsyncSessionLocal() as db:
                await log_action(
                    db,
        ...
        _bc_task.add_done_callback(lambda t: logger.debug("[WS] Broadcast error: %s", t.exception()) if not t.cancelled() and t.exception() else None)
        ...
        if not result["success"]:
            ...
            if is_non_critical:
                ...
                new_results.append({
                    ...
                    "agent": "system",
                    "success": True,
                    "output": {"action": "partial_failure_warning",
    ...
    if has_approval:
        ...
        if len(approval_results) > 1:
            logger.warning("[ORCHESTRATOR] %d pasos requieren aprobación simultánea — pausando en la primera", len(approval_results))
        return {
            **state,
            "plan": updated_plan,
            "agent_results": new_results,
