...
# ─── Estado compartido del grafo (importado desde módulo separado) ───────────
...
async def plan_node(state: OrchestratorState) -> dict:
    ...
    if domain == "coordinator":
        try:
            ...
            plan: list[SubTask] = []
            num_steps = len(plan_result.steps)
            for idx, step in enumerate(plan_result.steps):
                agent = step.agent if step.agent in VALID_DOMAINS else "unknown"
                # Construir dependencias desde needs_output_from del LLM
                raw_deps = getattr(step, "needs_output_from", None) or []
                # Validar: solo refs a pasos anteriores existentes (1-based)
                deps = [f"step_{d}" for d in raw_deps if isinstance(d, int) and 1 <= d <= idx]
                # Fallback: si el LLM no proporcionó deps y no es el primer paso,
                # usar heurística — si el agente anterior es distinto, sin dep (paralelo)
                if not deps and idx > 0 and not raw_deps:
                    # Conservador: encadenar linealmente como antes
                    deps = [f"step_{idx}"]
                plan.append({
                    "id": f"step_{idx+1}",
                    "agent": agent,
