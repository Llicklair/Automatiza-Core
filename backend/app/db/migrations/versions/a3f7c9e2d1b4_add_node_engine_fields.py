...
def upgrade() -> None:
    ...
    op.add_column(
        "workflow_executions", sa.Column("current_node_id", sa.String(100), nullable=True)
    )
    op.add_column(
        "workflow_executions", sa.Column("paused_at", sa.DateTime(timezone=True), nullable=True)
    )

    # PendingApproval: link to workflow execution
    op.add_column(
        "pending_approvals", sa.Column("execution_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
