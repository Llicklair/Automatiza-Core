...
def upgrade() -> None:
    ...
    op.add_column("employees", sa.Column("jornada_tipo", sa.String(20), server_default="completa", nullable=True))
    ...
    op.add_column("payrolls", sa.Column("pct_cc", sa.Numeric(5, 2), server_default="4.70", nullable=True))
    op.add_column("payrolls", sa.Column("pct_desempleo", sa.Numeric(5, 2), server_default="1.55", nullable=True))
    op.add_column("payrolls", sa.Column("pct_fp", sa.Numeric(5, 2), server_default="0.10", nullable=True))
    op.add_column("payrolls", sa.Column("pct_mei", sa.Numeric(5, 2), server_default="0.10", nullable=True))
    op.add_column("payrolls", sa.Column("pct_irpf", sa.Numeric(5, 2), server_default="15.00", nullable=True))
    op.add_column("payrolls", sa.Column("anticipos", sa.Numeric(10, 2), server_default="0", nullable=True))
...
def downgrade() -> None:
    ...
    for col in ["cuotas_empresa_json", "anticipos", "pct_irpf", "pct_mei", "pct_fp",
                "pct_desempleo", "pct_cc", "base_irpf", "base_cotizacion_cc",
                "devengos_json", "gross_salary"]:
    ...
    for col in ["bonificacion_tipo", "periodo_prueba_dias", "jornada_horas_semana", "jornada_tipo",
                "categoria_profesional", "convenio_colectivo", "tipo_contrato",
                "grupo_cotizacion", "numero_afiliacion_ss", "birth_date", "phone"]:
