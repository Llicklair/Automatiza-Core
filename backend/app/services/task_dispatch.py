...
async def dispatch_resume_orchestrator(task_id: str) -> None:
    ...
    await task_runner.submit("resume_orchestrator", resume_orchestrator(task_id), f"resume:{task_id}")


async def dispatch_node_engine(execution_id: str) -> None:
    from app.workers.tasks_node_engine import run_node_engine
    await task_runner.submit("run_node_engine", run_node_engine(execution_id), f"node:{execution_id}")
