...
def generate_payroll_pdf(payroll_data: dict, theme_config: dict | None = None) -> bytes:
    ...
    trab_info = [
        ...
        _info_row('N\u00ba Afil. SS:', employee.get('numero_afiliacion_ss', ''), S['small_bold'], S['small']),
        _info_row('Grupo Prof.:', employee.get('categoria_profesional') or employee.get('position', ''), S['small_bold'], S['small']),
    ...
    rows.append([Paragraph('Salario base', S['small']),
                 Paragraph(_eur(base), right_sty), ''])
    for concepto, importe in (devengos or {}).items():
        if concepto != 'salario_base' and float(importe or 0) != 0:
            ...
            rows.append([Paragraph(label, S['small']),
                         Paragraph(_eur(float(importe)), right_sty), ''])
    ...
    rows.append([
        ...
        Paragraph(f'<b>{_eur(total_dev)}</b>', ParagraphStyle('td', parent=S['small_bold'], alignment=TA_RIGHT)),
        '',
    ])
    ...
    for label, val in ded_items:
        rows.append([Paragraph(label, S['small']), '',
                     Paragraph(_eur(val), right_sty)])

    # B. TOTAL A DEDUCIR
    rows.append([
        Paragraph('<b>B. TOTAL A DEDUCIR</b>', S['small_bold']),
        '',
        Paragraph(f'<b>{_eur(total_ded)}</b>', ParagraphStyle('tdd', parent=S['small_bold'], alignment=TA_RIGHT)),
    ])
    ...
    elements.append(Paragraph(
        ...
        'Y CONCEPTOS DE RECAUDACI\u00d3N CONJUNTA</b>', S['small_bold']))
    ...
    bc_header = ['Concepto', 'Base', '% Empresa', 'Aport. Empresa', '% Trabajador', 'Aport. Trabajador']
    bc_rows = [bc_header]
    bc_rows.append(['Contingencias comunes', _eur(base_cc), '23,60', _eur(emp_cc),
                    f'{pct_cc:.2f}', _eur(ss_cc)])
    bc_rows.append(['AT y EP', _eur(base_cc), 'variable', _eur(emp_at), '', ''])
    bc_rows.append(['Desempleo', _eur(base_cc), '5,50', _eur(emp_des),
                    f'{pct_des:.2f}', _eur(ss_des)])
    bc_rows.append(['Formaci\u00f3n profesional', _eur(base_cc), '0,60', _eur(emp_fp),
                    f'{pct_fp:.2f}', _eur(ss_fp)])
    bc_rows.append(['MEI', _eur(base_cc), '0,50', _eur(emp_mei),
                    f'{pct_mei:.2f}', _eur(ss_mei)])
...
def generate_finiquito_pdf(finiquito_data: dict) -> bytes:
    ...
    # Conformidad
    elements.append(Paragraph(
        'Y en prueba de conformidad con cuanto antecede firmamos '
        'el presente finiquito:', S['body'],
    ))
    elements.append(Spacer(1, 2 * mm))
...
def generate_liquidacion_finiquito_pdf(liquidacion_data: dict) -> bytes:
    ...
    elements.append(Spacer(1, 6 * mm))
    elements.append(Paragraph(
        '<b>DOCUMENTO DE LIQUIDACI\u00d3N Y FINIQUITO</b>', S['title']))
    elements.append(Spacer(1, 8 * mm))
    ...
    trab_data = [
        ...
        ['N\u00ba Afil. SS:', employee.get('naf', ''), 'Fecha alta:', _format_date(employee.get('fecha_alta', ''))],
...
def generate_registro_jornada_pdf(registro_data: dict) -> bytes:
    ...
    elements.append(Spacer(1, 4 * mm))
    elements.append(Paragraph(
        '<b>Listado Resumen mensual del registro de jornada</b>', S['title']))
    elements.append(Spacer(1, 6 * mm))
    ...
    info_data = [
        ...
        ['Centro de trabajo:', company.get('centro_trabajo', ''),
         'Mes/A\u00f1o:', f'{_month_name_es(mes).capitalize()} {anio}'],
    ]
    ...
    # Tabla de registros
    header = ['D\u00cdA', 'HORA ENTRADA', 'HORA SALIDA',
              'HORAS ORDINARIAS', 'INCIDENCIAS', 'HORAS EXTRAS']
    rows = [header]
