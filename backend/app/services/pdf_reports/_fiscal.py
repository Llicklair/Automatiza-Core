"""
Generación de PDFs: Modelo 303 (IVA) e informe fiscal (IVA + IRPF + IS).
"""
...
def _modelo303_vat_table(rows: list, total_label: str, bg_color: str, s, C) -> tuple:
    ...
    table_data = [[Paragraph("Tipo IVA", s["header"]), Paragraph("Base imponible", s["header"]), Paragraph("Cuota", s["header"])]]
    total_base = 0.0
    total_quota = 0.0
    for row in rows:
    ...
    table_data.append([
        Paragraph(total_label, ParagraphStyle("VatTotalLbl", parent=s["styles"]["Normal"],
    ...
    tbl.setStyle(TableStyle(_table_header_style() + [
        ("ALIGN", (1, 0), (-1, -1), "RIGHT"),
...
def _modelo303_resultado(total_devengado: float, total_deducible: float, s, C) -> list:
    ...
    result_data = [
        [Paragraph("IVA devengado (A):", s["right"]), Paragraph(f"{total_devengado:.2f} €", s["right_bold"])],
        [Paragraph("IVA deducible (B):", s["right"]), Paragraph(f"{total_deducible:.2f} €", s["right_bold"])],
        [
            Paragraph(f"RESULTADO ({result_label}):",
                      ParagraphStyle("ResLabel", parent=sty, fontSize=12, fontName="Helvetica-Bold",
                                     textColor=colors.HexColor(result_color), alignment=TA_RIGHT)),
            Paragraph(f"{abs(resultado):.2f} €",
                      ParagraphStyle("ResVal", parent=sty, fontSize=14, fontName="Helvetica-Bold",
                                     textColor=colors.HexColor(result_color), alignment=TA_RIGHT)),
        ],
    ]
...
def generate_modelo_303_pdf(data: dict) -> bytes:
    ...
    # ── CABECERA ──
    elements.extend(_modelo303_info_header(
    ...
    elements.extend([
        ...
        Paragraph(
            ...
            ParagraphStyle("AEATFooter", parent=s["styles"]["Normal"], fontSize=8,
