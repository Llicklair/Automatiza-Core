...
IMPORT_TARGETS: dict[str, dict] = {
    "productos": {
        ...
        "fields": {
            ...
            "articulo": "name", "artículo": "name",
            "descripcion": "description", "descripción": "description", "description": "description",
            "sku": "sku", "referencia": "sku", "ref": "sku", "codigo": "sku", "código": "sku",
            "barcode": "barcode", "ean": "barcode", "codigo de barras": "barcode",
            "precio": "price", "pvp": "price", "price": "price", "precio venta": "price",
            "coste": "cost_price", "costo": "cost_price", "cost": "cost_price", "precio coste": "cost_price",
            "iva": "tax_percentage", "tax": "tax_percentage", "impuesto": "tax_percentage",
    ...
    "clientes": {
        ...
        "model": Client,
        "fields": {
            "nombre": "name", "name": "name", "cliente": "name", "empresa": "name",
            "razon social": "name", "razón social": "name",
            "nif": "nif", "cif": "nif", "dni": "nif",
            "email": "email", "correo": "email", "e-mail": "email",
            "telefono": "phone", "teléfono": "phone", "phone": "phone", "tel": "phone",
            "movil": "phone", "móvil": "phone",
            "direccion": "address", "dirección": "address", "address": "address",
            "ciudad": "city", "city": "city", "poblacion": "city", "población": "city",
            "cp": "postal_code", "codigo postal": "postal_code", "código postal": "postal_code",
    ...
    "empleados": {
        ...
        "model": Employee,
        "fields": {
            "nombre": "name", "name": "name", "empleado": "name",
            "nif": "nif", "dni": "nif",
            ...
            "salario": "base_salary", "salario base": "base_salary",
            "salario_base": "base_salary", "base_salary": "base_salary", "sueldo": "base_salary",
        },
        "required": ["name"],
        "numeric": {"base_salary"},
        "integer": set(),
...
async def _load_doc(db, tenant_id, document_id: uuid.UUID) -> TenantDocument:
    res = await db.execute(
        select(TenantDocument).where(
            TenantDocument.tenant_id == tenant_id, TenantDocument.id == document_id
        )
    )
...
def _resolve_target(target: str | None, columns: list[str]) -> tuple[str, dict]:
    ...
    if key not in IMPORT_TARGETS:
        raise ValueError(
            "No se pudo determinar el tipo de datos. Indica un target: "
            + ", ".join(IMPORT_TARGETS)
        )
