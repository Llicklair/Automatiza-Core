...
async def _entries_in_range(db: AsyncSession, tenant_id: UUID, start: date, end: date) -> list[JournalEntry]:
...
async def generate_balance_pyg_pdf(
    ...
    total_ingresos = -sum(
        (s for _, s in grupos.get("7", [])), Decimal("0")
    )  # Ingresos son haber → saldo negativo en debe-haber
    ...
    balance_rows = [
        ...
        [
            "A) Activo no corriente (2)",
            _fmt_eur(activo_no_corriente),
            "A) Patrimonio neto (1) + Rdo. ejercicio",
            _fmt_eur(patrimonio + resultado),
        ],
        ["B) Existencias (3)", _fmt_eur(existencias), "B) Pasivo no corriente", _fmt_eur(0)],
        ["C) Deudores (4 deudor)", _fmt_eur(deudores), "C) Acreedores (4 acreedor)", _fmt_eur(acreedores)],
        ["D) Tesorería (5 deudor)", _fmt_eur(tesoreria), "D) Deuda corto plazo (5 acreedor)", _fmt_eur(financiero_cp)],
        ["TOTAL ACTIVO", _fmt_eur(total_activo), "TOTAL PN + PASIVO", _fmt_eur(total_pasivo_pn)],
    ]
