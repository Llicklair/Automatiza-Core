...
export function CvAnalysisTab({
    ...
    const createCandidate = async () => {
        ...
        try {
            ...
            showToast("Candidato creado en la vacante", "success");
            reset();
            setPosId("");
            onCandidateCreated(posId);
        } catch (e: any) {
            showToast(e?.message ?? "No se pudo crear el candidato", "error");
        } finally {
            setCreating(false);
        }
    };
                ...
                <div className="bg-card border border-border rounded-xl overflow-hidden">
                    ...
                    <div className="border-t border-border bg-muted/30 px-6 py-4">
                        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5 mb-3">
                            <UserPlus className="w-3.5 h-3.5" /> Añadir como candidato
                        </h3>
                        {positions.length === 0 ? (
                            <p className="text-sm text-muted-foreground">
                                Crea primero una vacante en la pestaña «Vacantes y candidatos» para poder asignar este CV.
                            </p>
                        ) : (
