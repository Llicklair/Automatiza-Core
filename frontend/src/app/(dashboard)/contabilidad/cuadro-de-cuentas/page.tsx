...
export default function CuadroCuentasPage() {
    ...
    return (
        <div className="p-8 max-w-[1400px] mx-auto space-y-6">
            <PageHeader
                title={t("cuadroCuentas.title")}
                description={t("cuadroCuentas.description")}
                icon={FolderTree}
                actions={
                    <div className="flex items-center gap-3">
                        {noData && (
                            <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs px-3 py-2 rounded-xl">
                                <BookOpen className="w-4 h-4" /> {t("cuadroCuentas.noEntriesBadge")}
                            </div>
                        )}
                        <div className="relative">
                            <Search className="w-4 h-4 text-muted-foreground absolute left-3 top-1/2 -translate-y-1/2" />
                            <Input
                                placeholder={t("cuadroCuentas.searchPlaceholder")}
                                value={search}
            ...
            {loading ? (
                <div className="flex items-center justify-center py-24 text-muted-foreground gap-2">
                    <Loader2 className="w-5 h-5 animate-spin" /> {t("cuadroCuentas.loading")}
                </div>
                ...
                <div className="bg-card border border-border rounded-2xl p-16 flex flex-col items-center text-center">
                    ...
                    <h2 className="text-lg font-bold text-foreground mb-2">
                        {noData ? t("cuadroCuentas.emptyNoData") : t("cuadroCuentas.emptyNoResults")}
                    </h2>
                    <p className="text-sm text-muted-foreground max-w-md">
                        {noData
                            ? t("cuadroCuentas.emptyNoDataHint")
                            : t("cuadroCuentas.emptyNoResultsHint", { search })}
                    </p>
                </div>
            ) : (
                <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-xl shadow-black/20">
                    <div className="grid grid-cols-12 gap-4 px-6 py-3 border-b border-border text-xs font-medium text-muted-foreground uppercase tracking-wide bg-muted">
                        <div className="col-span-5">{t("cuadroCuentas.thAccount")}</div>
                        <div className="col-span-3 text-right">{t("cuadroCuentas.thDebit")}</div>
                        <div className="col-span-3 text-right">{t("cuadroCuentas.thCredit")}</div>
                        <div className="col-span-1 text-right">{t("cuadroCuentas.thBalance")}</div>
                    </div>
