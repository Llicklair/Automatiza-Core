...
export const CATEGORY_CONFIG: Record<string, { icon: string; color: string; label: string }> = {
    ...
    documents:  { icon: "📄", color: "bg-muted border-border text-muted-foreground",       label: "Documentos" },
    system:     { icon: "⚙️", color: "bg-muted border-border text-muted-foreground",       label: "Sistema" },
};
        ...
        <div className="flex gap-3 py-4 px-1 border-b border-border last:border-0 hover:bg-card rounded-lg transition-colors -mx-1 px-2">
            ...
            <div className="flex-1 min-w-0 space-y-1">
                ...
                <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground/60 shrink-0">{timeAgo(entry.created_at)}</span>
                </div>
