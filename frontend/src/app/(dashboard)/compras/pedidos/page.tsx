...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string; border: string }> = {
    ...
    confirmed: { label: "Confirmado", color: "text-indigo-400", bg: "bg-indigo-500/10", border: "border-indigo-500/20" },
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Pedidos de Compra</h1>
                    <p className="mt-1 text-sm text-zinc-400">Gestiona los pedidos a tus proveedores antes de recibir la factura.</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                {[
                    ...
                    { label: "Total comprometido", value: fmt(orders.filter(o => o.status !== "cancelled").reduce((a, o) => a + o.amount_total, 0)), color: "text-white" },
                ].map(stat => (
            ...
            {loading ? (
                ...
                <div className="bg-[#111113] border border-[#27272a] rounded-2xl p-16 flex flex-col items-center text-center">
                    ...
                    <h2 className="text-lg font-bold text-white mb-2">{orders.length === 0 ? "Sin pedidos de compra" : "Sin resultados"}</h2>
                    <p className="text-sm text-zinc-500 max-w-sm">
                        {orders.length === 0 ? "Crea tu primer pedido de compra a un proveedor." : `Sin resultados para "${search}"`}
                    </p>
                    {orders.length === 0 && (
                        <button onClick={openNew} className="mt-6 bg-indigo-600 hover:bg-indigo-500 text-white text-sm px-4 py-2 rounded-xl transition-colors">Crear pedido</button>
                    )}
                </div>
            ) : (
                            ...
                            <div key={order.id} className="bg-[#111113] border border-[#27272a] rounded-2xl overflow-hidden">
                                <div className="flex items-center gap-4 px-6 py-4">
                                    <button onClick={() => setExpandedId(isExpanded ? null : order.id)} className="flex-1 flex items-center gap-4 text-left">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-zinc-500 mt-0.5">{order.supplier?.name || "Proveedor desconocido"}</p>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        {nextStatus && (
                                            <button onClick={() => handleAdvance(order)} className="flex items-center gap-1.5 bg-emerald-600/80 hover:bg-emerald-500 text-white text-xs px-3 py-1.5 rounded-lg transition-colors">
                                                <Check className="w-3 h-3" /> {STATUS_MAP[nextStatus]?.label}
                                            </button>
                                        )}
                                        {!["cancelled", "received"].includes(order.status) && (
                                            <button onClick={() => handleCancel(order)} className="text-xs text-zinc-500 hover:text-rose-400 px-2 py-1.5 rounded-lg hover:bg-rose-500/10 transition-colors">Cancelar</button>
                                        )}
                                ...
                                {isExpanded && (
                                    <div className="border-t border-[#27272a]/50 px-6 py-4 bg-[#161618]/40">
                                        ...
                                        {order.notes && <p className="text-xs text-zinc-600 mt-3 italic">{order.notes}</p>}
                                    </div>
                                )}
                            </div>
                        );
                ...
                <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm pt-16 pb-8 overflow-y-auto">
                    <div className="bg-[#111113] border border-[#27272a] rounded-2xl p-8 w-full max-w-2xl shadow-2xl">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-bold text-white">Nuevo pedido de compra</h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs text-zinc-400 mb-1.5 font-medium">Proveedor *</label>
                                        ...
                                        <div key={i} className="grid grid-cols-12 gap-2 items-end bg-[#161618] rounded-xl p-3">
                                            ...
                                            <div className="col-span-1 flex items-end justify-end">
                                                <button type="button" onClick={() => setForm(f => ({ ...f, lines: f.lines.filter((_, idx) => idx !== i) }))} disabled={form.lines.length === 1}
                                                    className="p-2 rounded-lg hover:bg-rose-500/10 text-zinc-600 hover:text-rose-400 transition-colors disabled:opacity-30">
                                                    <X className="w-3.5 h-3.5" />
                                                </button>
                                            </div>
                                            <div className="col-span-12 text-right text-xs text-zinc-500">
                                                Total: <span className="text-zinc-300 font-mono">{fmt(lineTotal(line))}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
