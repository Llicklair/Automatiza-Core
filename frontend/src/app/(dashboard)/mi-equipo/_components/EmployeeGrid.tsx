...
export function EmployeeGrid({ employees, loading, onRefresh, refreshing }: EmployeeGridProps) {
    ...
    const handleSeed = async () => {
        setSeeding(true);
        try { await api.aiEmployees.seed(); onRefresh(); showToast("Equipo inicial creado"); }
        catch { showToast("Error al crear equipo inicial"); }
    ...
    const handleToggle = async (id: string, current: AIEmployee["status"]) => {
        ...
        try {
            ...
            showToast("Error al cambiar estado del agente");
        }
    };
    ...
    return (
        <>
            ...
            {instructTarget && <InstructModal employee={instructTarget} onClose={() => setInstructTarget(null)}
                onSent={() => { showToast(`Instruccion enviada a ${instructTarget.name}`); onRefresh(); }} />}
            {showNewModal && <NewEmployeeModal onClose={() => setShowNewModal(false)}
                onCreated={() => { onRefresh(); showToast("Empleado IA creado"); }} />}
            ...
            {employees.length === 0 ? (
                <div className="text-center py-16 border-2 border-dashed border-border rounded-xl">
                    ...
                    <p className="text-muted-foreground font-medium text-sm">Sin empleados aun</p>
                    <p className="text-xs text-muted-foreground mt-1">Crea el equipo inicial con Ana, Carlos y Sofia</p>
